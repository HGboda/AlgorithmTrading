...
def analysisPort(widget):
    ...
    corrdfs = pd.DataFrame({'title':selecttitles,'corrval':corrs,'min':mins,'max':maxs,'closemins':closemins,'closemaxs':closemaxs,'pctcum':pctcums,'stdbarscum':stdbarscums\
        ...
        ,columns = ['title','corrval','upperPrice','Price','lowerPrice','bench','currentgain','min','max','closemins','closemaxs','pctcum','stdbarscum','bar50p','stds','buydate'\
        ,'gainmoney'])
    ...
    print 'port corr mean:',corrdfs['corrval'].mean()
