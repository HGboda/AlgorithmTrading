...
def portfoliosell(widget):
    ...
    if os.path.isfile(title):
        ...
        try:
            ...
            if buyportdf[buyportdf['title'] == stocktitle]:
...
def curStatus(widget):
    ...
    try:
        for cnt in range(sheet.nrows):
            if sheet.row_values(cnt)[1] == stocktitle:
    ...
    print selectbars.tail()
    # display(HTML(selectbars.to_html()))
...
run_button4 = widgets.ButtonWidget(description="Current Price")        
