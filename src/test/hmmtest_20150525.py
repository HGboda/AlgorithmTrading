        ...
        for tmpday in range(extrapolation_Startday,day+1,1):
            ...
            if len(extraMinDayarrs) > 0:
                ...
                if len(extraMaxDayarrs) > 0:
                    ...
                    max_bench_sol_X  = (g_interFallYoutVal - param_MinSlope[1])/param_MinSlope[0]                                
                    if g_interFallYoutVal < bars['bench'][tmpday] and gfilterbars['CloseMAsig'][tmpday] == 1\
                        and paramfit_bench_predict[0] > 0.0 and ma3benchAbovePct > 0.7 and param_MinSlope[0] >= 0.00 and paramfit_bench_dir[0] >= 0.00\
                        and yup_bench_lim_next - yup_bench_lim_prev >= 0.02 :
                        # and y_bench_lowlim_next - y_bench_lowlim_prev >= 0.02:
                        
                        if param_MinSlope[0] > paramfit_bench_predict[0] and param_MinSlope[0] < param_MaxSlope[0] and solv_intsec_MinMaxDir < tmpday:
                            disbarsdf = pd.DataFrame({ 'hold_False_1':[False]\
                            ...
                            , columns= ['hold_False_1','g_interFallYoutVal','bench','tmpday'\
                        ...
                        disbarsdf = pd.DataFrame({ 'hold_True_1':[True]\
                            ,'g_interFallYoutVal':[g_interFallYoutVal],'bench':[bars['bench'][tmpday]]\
                        ,'tmpday':[tmpday] ,'CloseMAsig':[gfilterbars['CloseMAsig'][tmpday]]\
                        ,'ma3benchAbovePct':[ma3benchAbovePct],'param_MinSlope':[param_MinSlope[0]]\
                        ,'paramfit_bench_predict':[paramfit_bench_predict[0]]\
                        ...
                        ,'bench-g_interFallYoutVal':[bars['bench'][tmpday] - g_interFallYoutVal]\
                        ,'max_bench_sol_X':[max_bench_sol_X]} 
                        , columns= ['hold_True_1','g_interFallYoutVal','bench','tmpday'\
                            ,'CloseMAsig','ma3benchAbovePct','param_MinSlope','paramfit_bench_predict'\
                            ,'param_setBenchSlope','param_MaxSlope','next-prev','tmp_gainsum'\
                            ,'bench-g_interFallYoutVal','max_bench_sol_X'])
                        display(HTML(disbarsdf.to_html()))
                        ...
                        if g_interFallYoutVal < bars['bench'][tmpday] and gfilterbars['CloseMAsig'][tmpday] == 1\
                            and paramfit_bench_predict[0] > 0.0 and param_MinSlope[0] > paramfit_bench_predict[0]\
                            and (solv_intsec_MinMaxDir < tmpday - 30 or solv_intsec_MinMaxDir > tmpday + 30):

                            tradesignals.append(tmpday)
                            if gTradeStatus == False:
                                gTradeStatus = True

                            disbarsdf = pd.DataFrame({ 'hold_True_2':[True]\
                                ,'g_interFallYoutVal':[g_interFallYoutVal],'bench':[bars['bench'][tmpday]]\
                            ,'tmpday':[tmpday] ,'CloseMAsig':[gfilterbars['CloseMAsig'][tmpday]]\
                            ,'ma3benchAbovePct':[ma3benchAbovePct],'param_MinSlope':[param_MinSlope[0]]\
                            ,'paramfit_bench_predict':[paramfit_bench_predict[0]]\
                            ...
                            ,'bench-g_interFallYoutVal':[bars['bench'][tmpday] - g_interFallYoutVal]\
                            ,'max_bench_sol_X':[max_bench_sol_X]} 
                            , columns= ['hold_True_2','g_interFallYoutVal','bench','tmpday'\
                                ,'CloseMAsig','ma3benchAbovePct','param_MinSlope','paramfit_bench_predict'\
                                ,'param_setBenchSlope','param_MaxSlope','next-prev','tmp_gainsum'\
                                ,'bench-g_interFallYoutVal','max_bench_sol_X'])
                            display(HTML(disbarsdf.to_html()))    
                            # print 'interFallXinArrs == 0 g_interFallXinDay',g_interFallXinDay,'g_interFallYoutVal',g_interFallYoutVal,bars['bench'][tmpday],'tmpday',tmpday ,'CloseMAsig',\
                            #     gfilterbars['CloseMAsig'][tmpday],'param_MinSlope',param_MinSlope[0],'paramfit_bench_predict',paramfit_bench_predict[0],\
                            #     'param_setBenchSlope',param_setBenchSlope[0],'param_MaxSlope',param_MaxSlope[0]
                            # print ''
                            continue
